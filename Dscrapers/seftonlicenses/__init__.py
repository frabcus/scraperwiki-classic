...
def Scrape():
    ...
    br.open(url)
    ff = list(br.forms())[0]
    rr = ff.click(name="Template$ctl10$ctl00$BtnGenLic")
    br.open(url, rr.get_data())
    #SaveScraping(scraper_tag="seftonlicenses", name="formpagegeneral", url=url, text=br.response().read())
    ...
    #SaveScraping(scraper_tag="seftonlicenses", name="formpagegeneralG", url=url, text=br.response().read())
    ...
    for i in range(0, 30):
        ...
        SaveScraping(scraper_tag="seftonlicensegneral", name="page %02d" % i, url=url, text=br.response().read())
