...
class LatestScrapersByTag(Feed):
    ...
    def title(self, obj):
        return "ScraperWiki.com: Scrapers tagged with '%s'" % obj.name
...
class LatestScrapers(Feed):
    title = "Latest ScraperWiki scrapers"
    link = "/scrapers/list/"
    ...
    def items(self):
        return Scraper.objects.order_by('-created_at')[:10]
...
class LatestScrapersBySearchTerm(Feed):
    ...
    def title(self, obj):
        return "ScraperWiki.com: Scrapers matching '%s'" % obj
